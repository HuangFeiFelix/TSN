...
/*int pci_connect(device_t *igb_dev)
{
	...
	for (dev = pacc->devices; dev; dev = dev->next) {
		...
		snprintf(devpath, IGB_BIND_NAMESZ, "%04x:%02x:%02x.%d",
				...
				printf("%s\t", devpath);
				printf("%x\t",dev->vendor_id);
				printf("%x\n",dev->device_id);
		err = igb_probe(igb_dev);																										//Realiza sondeos para encontrar el dispositivo conectado al bus pci que conincida
		if (err) {																																	//con el dispositivo que usa el driver igb
			continue;
		}
